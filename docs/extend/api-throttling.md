...
DuRoom comes with a builtin `DuRoom\Api\Middleware\ThrottleApi` [middleware](middleware.md) for throttling requests to the API.
...
Throttlers will be run on EVERY request, and are responsible for figuring out whether or not they apply. For example, consider DuRoom's post throttler:
