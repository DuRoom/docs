...
DuRoom includes a powerful notification system to alert users to new activity.

## Notification Types

### Defining a Notification Type

To define a notification type, you will need to create a new class which implements `DuRoom\Notification\Blueprint\BlueprintInterface`. This class will define your notification's content and behaviour through the following methods:
...
Lets take a look at an example from [DuRoom Likes](https://github.com/DuRoom/likes/blob/master/src/Notification/PostLikedBlueprint.php):
...
Take a look at [`DiscussionRenamedBlueprint`](https://github.com/DuRoom/framework/blob/main/framework/core/src/Notification/Blueprint/DiscussionRenamedBlueprint.php) if you want another example.
...
Lets look at an example from [DuRoom Subscriptions](https://github.com/DuRoom/subscriptions/blob/master/extend.php):
...
To do this, your notification blueprint should implement [`DuRoom\Notification\MailableInterface`](https://api.duroom.js.org/docs/php/master/duroom/notification/mailableinterface) in addition to [`DuRoom\Notification\Blueprint\BlueprintInterface`](https://api.duroom.js.org/docs/php/master/duroom/notification/blueprint/blueprintinterface).
...
Let's take a look at an example from [DuRoom Mentions](https://github.com/DuRoom/mentions/blob/master/src/Notification/PostMentionedBlueprint.php)
...
The driver should implement `DuRoom\Notification\Driver\NotificationDriverInterface`. Let's look at an annotated example from the [Pusher extension](https://github.com/DuRoom/pusher/blob/master/src/PusherNotificationDriver.php):
...
Another example from [DuRoom Pusher](https://github.com/DuRoom/pusher/blob/master/extend.php):
...
As with everything in DuRoom, what we register in the backend, must be registered in the frontend as well.

Similar to the notification blueprint, we need tell DuRoom how we want our notification displayed.
...
From [DuRoom Subscriptions](https://github.com/DuRoom/subscriptions/blob/master/js/src/forum/components/NewPostNotification.js), when a new post is posted on a followed discussion:
...
Next, we need to tell DuRoom that the notification you send in the backend corresponds to the frontend notification we just created.
...
Let's give users an option to change their settings for your notification. All you have to do is extend the [`notificationGrid`](https://github.com/DuRoom/framework/blob/main/framework/core/js/src/forum/components/NotificationGrid.js)'s [`notificationTypes()`](https://github.com/DuRoom/framework/blob/main/framework/core/js/src/forum/components/NotificationGrid.js#L204) function

From [DuRoom-Likes](https://github.com/DuRoom/likes/blob/master/js/src/forum/index.js):
...
Thankfully, this is the easiest part, simply use[`NotificationSyncer`](https://github.com/DuRoom/framework/blob/main/framework/core/src/Notification/NotificationSyncer.php)'s sync function. It accepts 2 arguments:
...
From [DuRoom Likes](https://github.com/DuRoom/likes/blob/master/src/Listener/SendNotificationWhenPostIsLiked.php):
...
*Tried everything?* Well if you've tried everything then I guess... Kidding. Feel free to post in the [DuRoom Community](https://discuss-duroom.js.org/t/extensibility) and someone will be around to lend a hand!
