...
We provide `duroom/common/components/Page` as a base class for pages in both the `admin` and `forum` frontends. It has a few benefits:
...
DuRoom uses a setting to determine which page should be the homepage: this gives admins flexibility to customize their communities.
...
An example from the [Tags extension](https://github.com/DuRoom/tags/blob/master/js/src/admin/addTagsHomePageOption.js):
...
To allow this, DuRoom creates (and stores) instances of [`PageState`](https://api.duroom.js.org/docs/js/master/class/src/common/states/pagestate.js~pagestate) as `app.current` and `app.previous`.
These store:
...
For example, this is how the Discussion Page makes its [`PostStreamState`](https://api.duroom.js.org/docs/js/master/class/src/forum/states/poststreamstate.js~poststreamstate) instance globally available.
...
```jsx
import IndexPage from 'duroom/forum/components/DiscussionPage';
...
DuRoom actually already wraps all its components in the `duroom/common/resolvers/DefaultResolver` resolver. This has the following benefits:
...
- the `resolver` key can be used to provide an **instance** of a route resolver. This instance should define which component should be used, and hardcode the route name to be passed into it. This instance will be used without any modifications by DuRoom.
- The `resolverClass` key AND `component` key can be used to provide a **class** that will be used to instantiate a route resolver, to be used instead of DuRoom's default one, as well as the component to use. Its constructor should take 2 arguments: `(component, routeName)`.
...
We strongly encourage custom route resolvers to extend `duroom/common/resolvers/DefaultResolver`.
For example, DuRoom's `duroom/forum/resolvers/DiscussionPageResolver` assigns the same key to all links to the same discussion (regardless of the current post), and triggers scrolling when using `m.route.set` to go from one post to another on the same discussion page:
